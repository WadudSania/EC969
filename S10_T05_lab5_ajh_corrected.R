...
sink(file = "lab5_corrected.log", split = T)   #open a log file or can use logr package
...
# Nicer, including standard errors
 ...
 #   minimum value, lower-hinge value (first quartile /  median of lower half), median value, upper-hinge value and maximum value of the input data
  ...
  # We get an error message if we try to look at the marginal effects of those variables as though they are numeric:
  summary(margins(logit3, variables="lag.coh", at = list(lag.anykids = 1)))
  
  # Try instead explicitly making clear this is a factor variable (by enclosing in quotes) 
  summary(margins(logit3, variables="lag.coh", at = list(lag.anykids = '1')))
  summary(margins(logit3, variables="lag.coh", at = list(lag.anykids = '0')))
  
  summary(margins(logit3, variables="lag.anykids", at = list(lag.coh = '1')))
  summary(margins(logit3, variables="lag.anykids", at = list(lag.coh = '0')))
  
 
 

   # Alternative way to see how the marginal effects of the discrete change is to plot
    # predicted values and compare those at 0 and 1
    
  # Plot prediction of cohabitation, by anykids
 cplot(logit3, "lag.anykids",   what = "prediction", main = "Predicted values, by having any kids")
  
  # Plot prediction of anykids, by cohabitation
  cplot(logit3, "lag.coh",   what = "prediction", main = "Predicted values, by cohabitation")  
